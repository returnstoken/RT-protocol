...
contract RegulatedToken is DetailedERC20, MintableToken {
   ...
   *         `GovernorService` contract responsible for checking trade
   *         permissions.
   */
  ...
  /**
   * @notice Retreives the address of the `GovernorService` that manages this token.
   *
   * @dev This function *MUST NOT* memoize the `GovernorService` address.  This would
   *      break the ability to upgrade the `GovernorService`.
   *
   * @return The `GovernorService` that manages this token.
   */
  function _service() constant public returns (GovernorService) {
    return GovernorService(registry.service());
  }
}
