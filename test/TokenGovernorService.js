...
const MockGovernorToken = artifacts.require('../test/helpers/MockGovernorToken.sol');
const TokenGovernorService = artifacts.require('./TokenGovernorService.sol');
...
contract('TokenGovernorService', async accounts => {
  let owner;
  let spender;
  let account;
  let token;
  ...
  beforeEach(async () => {
    ...
    service = await TokenGovernorService.new({ from: owner });
    token = await MockGovernorToken.new(service.address);
  });
